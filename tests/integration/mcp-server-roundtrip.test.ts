...
function createMockBackend(): MemtraceBackend {
  return {
    execute: vi.fn().mockResolvedValue({
      ...
      data: {
        results: [
          { name: 'authenticateUser', file_path: 'src/auth.ts', start_line: 10, end_line: 20 },
        ],
      },
      trace_id: 't1',
      elapsed_ms: 1,
    ...
    listTools: vi.fn().mockResolvedValue([
      {
        ...
        description: 'Find code by symbol name',
        inputSchema: { type: 'object', properties: { query: { type: 'string' } } },
      },
      {
        name: 'memtrace_get_symbol_context',
        description: 'Get 360-degree context',
...
describe('MCP server roundtrip', () => {
  ...
  it('[P0] [AC1] tools/list returns runtime-discovered tool schemas matching backend.listTools()', async () => {
    ...
    // Then: the server returns those 2 tools plus the built-in telemetry tool
