...
function createMockBackend(tools: Array<{ name: string; description: string; inputSchema: Record<string, unknown> }> = []): MemtraceBackend {
  return {
    execute: vi.fn().mockResolvedValue({ tool: 'test', data: {}, trace_id: 't1', elapsed_ms: 1, degraded: false }),
    probe: vi.fn().mockResolvedValue(true),
    listTools: vi.fn().mockResolvedValue(tools),
  };
}

function createMockAdapter(dispatchResponse?: { content: Array<{ type: 'text'; text: string }> }): ToolProvider {
  return {
    dispatch: vi.fn().mockResolvedValue(
      dispatchResponse ?? { content: [{ type: 'text', text: 'result' }] },
    ),
  };
}
...
describe('createMcpServer', () => {
  ...
  it('[P0] [AC1] start() calls backend.listTools() and tools are discoverable via MCP client', async () => {
    const tools = [
      { name: 'memtrace_find_code', description: 'Find code', inputSchema: { type: 'object', properties: { query: { type: 'string' } } } },
      { name: 'memtrace_get_symbol_context', description: 'Get context', inputSchema: { type: 'object', properties: { symbol: { type: 'string' } } } },
    ];
  ...
  it('[P0] [AC2] tool handler dispatches message to adapter and returns content via MCP roundtrip', async () => {
    ...
    const backend = createMockBackend(tools);
    const dispatch = vi.fn().mockResolvedValue({ content: [{ type: 'text', text: 'fused result' }] });
  ...
  it('[P0] [AC3] classification error triggers passthrough via backend.execute()', async () => {
    const tools = [
      { name: 'memtrace_unknown_tool', description: 'Unknown tool', inputSchema: {} },
    ];
    const errorContent = { content: [{ type: 'text', text: JSON.stringify({ cause: 'classification_failed' }) }] };
    const backend = createMockBackend(tools);
