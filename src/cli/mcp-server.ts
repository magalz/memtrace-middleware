...
function isClassificationError(response: AgentResponse): boolean {
  ...
  try {
    ...
    return (
      parsed?.cause === 'classification_failed' || parsed?.cause === 'classification_low_confidence'
    );
  } catch {
    return false;
  }
  ...
  return async (args: Record<string, unknown>) => {
    ...
    try {
      ...
      return {
        ...
        ...(response.metadata ? { _meta: response.metadata } : {}),
      };
    } catch (err: unknown) {
  ...
  return {
    ...
    async start(transport?: Transport) {
      ...
      mcpServer.registerTool(
        ...
        {
          description:
            'Returns structured telemetry data with 5 core KPIs: query success rate, override frequency, latency percentiles, Memtrace uptime, and confidence distribution.',
          inputSchema: z.object({}).passthrough(),
        },
        async () => {
          const { buildTelemetryResponse } = await import('../telemetry/api.js');
          return {
            content: [{ type: 'text' as const, text: JSON.stringify(buildTelemetryResponse()) }],
          };
        }
      );
