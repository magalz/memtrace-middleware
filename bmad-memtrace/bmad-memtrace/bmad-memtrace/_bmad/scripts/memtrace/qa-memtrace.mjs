  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--blast-radius' && i + 1 < args.length) {
      ...
      if (!Number.isInteger(raw)) {
        console.error(`WARNING: threshold ${raw} truncated to ${Math.floor(raw)}`);
      }
      result.threshold = Math.floor(raw);
    } else {
...
function compute(blastData, coverageData, threshold) {
  ...
  for (const sym of blastData.affected_symbols) {
    if (sym.file == null || sym.name == null) continue;
  ...
  for (const mod of coverageData.modules) {
    ...
    } else if ((mod.coverage || '').trim().toLowerCase().startsWith('partial:')) {
      const n = Math.max(0, parseInt(cov.split(':')[1], 10) || 0);
