...
test('readJsonFile with non-existent file throws "File not found"', () => {
  ...
  try {
    execFileSync(
      process.execPath,
      [SCRIPT_PATH, '--blast-radius', missingPath, '--test-coverage', missingPath],
...
test('normalizeBlastData accepts evolved format (symbols instead of affected_symbols)', () => {
  const br = {
    target: 'test',
    risk_level: 'Low',
    symbols: [{ name: 'foo', file: 'src/a.ts', depth: 1 }],
    total_affected: 1,
  };
  const tc = makeTc([
    {
...
test('normalizeCoverageData accepts evolved format (file + status + covered_symbols)', () => {
  const br = makeBr([{ name: 'foo', file: 'src/a.ts', depth: 1 }]);
  const tc = {
    coverage: { modules: [{ file: 'src/a.ts', status: 'Yes', covered_symbols: ['foo'] }] },
  };
  const r = runScript(br, tc);
  assert.equal(r.exitCode, 0);
  assert.equal(r.output.passed, true);
