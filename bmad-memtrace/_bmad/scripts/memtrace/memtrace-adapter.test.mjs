...
describe('memtrace-adapter.mjs', () => {
  ...
  describe('McpClient robustness (unit)', () => {
    ...
    describe('debug instrumentation', () => {
      ...
      it('MEMTRACE_DEBUG=1 adapter should emit debug to stderr', { timeout: 30000 }, async () => {
        const r = await runAdapter(['--query', 'list_repos']);
        // When MEMTRACE_DEBUG is set, stderr should contain [McpClient] entries
        // But default adapter run should have clean stderr (only [FRESHNESS] possibly)
        // This test verifies the debug guard works correctly
        if (r.code === 0) {
